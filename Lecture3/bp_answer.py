...
for i in range(10000):
    ...
    dloss_dsig_ho = (y - sig_ho) # (4, 1)
    dsig_ho_dlayer_ho = sigmoid_derivative(sig_ho) # (4, 1)
    dlayer_ho_dw_ho = sig_ih # (4, 2)
    dlayer_ho_dsig_ih = weights_ho # (2, 1)
    dsig_ih_dlayer_ih = sigmoid_derivative(sig_ih) # (4, 2)
    dlayer_ih_dw_ih = x # (4, 2)
    
    dloss_dlayer_ho = dloss_dsig_ho * dsig_ho_dlayer_ho # (4, 1) * (4, 1) = (4, 1)
    dloss_dsigmoid_ih = dloss_dlayer_ho @ dlayer_ho_dsig_ih.T # (4, 1) @ (1, 2) = (4, 2)
    dloss_dlayer_ih = dloss_dsigmoid_ih * dsig_ih_dlayer_ih # (4, 2) * (4, 2) = (4, 2)
        
    dloss_dweights_ho = dlayer_ho_dw_ho.T @ dloss_dlayer_ho # (2, 4) @ (4, 1) -> (2, 1)
    dloss_dweights_ih = dlayer_ih_dw_ih.T @ dloss_dlayer_ih # (2, 4) @ (4, 2) -> (2, 2)
